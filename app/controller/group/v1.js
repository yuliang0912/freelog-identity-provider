...
module.exports = class UserGroupController extends Controller {
    ...
    async create(ctx) {
        ...
        let groupId = ctx.checkBody('groupId').exist().isGroupId().value
        ...
        if (groupType === 1 && !groupId.startsWith('group_user')) {
            ctx.error({msg: 'groupId命名不符合规则'})
        }
        if (groupType === 2 && !groupId.startsWith('group_node')) {
            ctx.error({msg: 'groupId命名不符合规则'})
        }

        members.forEach(parseInt)

        await ctx.service.groupService.createGroup({groupId, groupName, members, groupType})
